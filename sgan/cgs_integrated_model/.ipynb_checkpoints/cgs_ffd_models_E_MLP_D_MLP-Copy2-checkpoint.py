...
def make_mlp(dim_list, activation='relu', batch_norm=True, dropout=0):
    ...
    for dim_in, dim_out in zip(dim_list[:-2], dim_list[1:-1]):
        ...
        print("look hereeeeeeeeeeeeeeee"+ str(dropout))
        if dropout > 0:
            print("look hereeeeeeeeeeeeeeee" + str(dropout))
...
class Encoder(nn.Module):
    ...
    def __init__(
        ...
        dropout=0.0, pos_embed=True, pos_embed_freq=100, num_mlp_layers = 2
    ):
...
#################################################################################
        ...
        self.encoder_1 = nn.Sequential(nn.Linear(embedding_dim*obs_len, 4*h_dim), nn.Dropout(p=0.2),  nn.Linear(h_dim*4, h_dim))
        print(self.encoder_1)
        encoder_dims = [embedding_dim*obs_len] + [4*h_dim for i in range(self.num_mlp_layers-1)] + [h_dim]
        print("LLLLLLLLLLLLLLLLLLL"+str(encoder_dims))
...
class Decoder(nn.Module):
    ...
    def __init__(
        ...
        neighborhood_size=2.0, grid_size=8, num_mlp_layers=9
    ):
...
#################################################################################
        ...
        #print("LLLLLLLLLLLLLLLLLLLLLooooooooook here"+str(decoder_dims))
...
class TrajectoryGenerator(nn.Module):
    def __init__(
        ...
        pos_embed=1,  pos_embed_freq=100, num_mlp_decoder_layers = 9, num_mlp_encoder_layers = 2
    ):
        ...
        self.num_mlp_decoder_layers = num_mlp_decoder_layers
        self.num_mlp_encoder_layers = num_mlp_encoder_layers
...
class TrajectoryDiscriminator(nn.Module):
    def __init__(
        ...
        d_type='local', num_mlp_encoder_layers = 2, mlp_discriminator_layers = 5
    ):
        ...
        self.d_type = d_type
        self.num_mlp_encoder_layers = num_mlp_encoder_layers
        self.mlp_discriminator_layers = mlp_discriminator_layers
