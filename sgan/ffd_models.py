...
def make_mlp_bn(dim_list, activation='relu', batch_norm=True, dropout=0):
    layers = []
    batch_norm=True
    dropout=0.0
    for dim_in, dim_out in zip(dim_list[:-2], dim_list[2:-1]):
        layers.append(nn.Linear(dim_in, dim_out))
        if batch_norm:
            layers.append(nn.BatchNorm1d(dim_out))
        if activation == 'relu':
...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embedding_dim, pos_embed_freq=100, obs_len=8, dropout=0):
        ...
        div_term = torch.exp(torch.arange(0, embedding_dim, 2) *
                             -(math.log(pos_embed_freq) / embedding_dim))
...
class Encoder(nn.Module):
    ...
    def __init__(
        self, embedding_dim=64, rep_dim=64, h_dim=64, mlp_dim=1024, num_layers=1, obs_len=8,
        dropout=0.0, pos_embed=True, pos_embed_freq=100, activation='relu', batch_norm=False
    ):
        ...
        encoder_mlp_dims = [self.embedding_dim*obs_len] + [int(i) for i in list(self.h_dim*np.ones(self.num_layers, dtype=np.int64))] + [rep_dim]
        print(encoder_mlp_dims)
...
class Decoder(nn.Module):
    ...
    def __init__(
        ...
        # BEST Feedforward Architecture CGS
        # decoder_dims = [h_dim + embedding_dim, 64, 64, 8*embedding_dim]
        
        encoder_mlp_dims = [self.embedding_dim + self.h_dim] + [int(i) for i in list(self.h_dim*np.ones(self.num_layers, dtype=np.int64))] + [self.pred_len*self.embedding_dim]
...
class TrajectoryGenerator(nn.Module):
    def __init__(
        self, obs_len, pred_len, embedding_dim=64, encoder_h_dim=64, rep_dim=64,
        decoder_h_dim=128, mlp_dim=1024, num_layers=1, noise_dim=(0, ),
        noise_type='gaussian', noise_mix_type='ped', pooling_type=None,
        pool_every_timestep=True, dropout=0.0, bottleneck_dim=1024,
        activation='relu', batch_norm=True, neighborhood_size=2.0, grid_size=8,
        pos_embed=1,  pos_embed_freq=100, encoder_num_layers=1, decoder_num_layers=1
    ):
        ...
        # Decoder Hidden
        if pooling_type:
            input_dim = rep_dim + bottleneck_dim
        else:
            input_dim = rep_dim
    ...
    def mlp_decoder_needed(self):
        if (
            ...
            self.rep_dim != self.decoder_h_dim
        ):
            return True
        else:
            return False
    ...
    def forward(self, obs_traj, obs_traj_rel, seq_start_end, user_noise=None):
        ...
        if self.pooling_type is not None:
            # print("Pooling")
            pooling_input = final_encoder_h.view(-1, self.rep_dim).unsqueeze(0)
            pooling_output = self.pool_net(pooling_input, seq_start_end, last_pos_rel)
            mlp_decoder_context_input = torch.cat([final_encoder_h.view(-1, self.rep_dim), pooling_output], dim=1)
        else:
            mlp_decoder_context_input = final_encoder_h.view(-1, self.rep_dim)
...
class TrajectoryDiscriminator(nn.Module):
    def __init__(
        ...
        real_classifier_dims = [(obs_len + pred_len) * 2] + [int(i) for i in list(self.h_dim*np.ones(self.num_layers))] + [1]
    ...
    def forward(self, traj, traj_rel, seq_start_end=None):
        ...
        #print("看这里"+str(classifier_input.shape))
