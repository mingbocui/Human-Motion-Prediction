...
parser.add_argument('--dataset_name', default='three_traj',type=str, help = "three_traj or five_traj")
...
parser.add_argument('--Decoder_type', default='MLP', type=str)

parser.add_argument('--mlp_encoder_layers', default=2, type=int)
parser.add_argument('--mlp_decoder_layers', default=9, type=int)
parser.add_argument('--mlp_discriminator_layers', default=5, type=int)

#parser.add_argument('--traj_type', default='three_traj', type=str, help = "three_traj or five_traj")
        ...
        for batch in loader:
            ...
            for k in range(100):
                ...
                if k == 0:
                    ...
                    view_traj(ax, pred_traj_fake_permuted[0,:,:], pred_traj_gt_permuted[0,:,:], obs_traj_permuted[0,:,:], args)########changed by cuimingbo
                    
                #if k == 0:
                   # view_traj(ax, pred_traj_gt_permuted[0,:,:], pred_traj_gt_permuted[0,:,:], obs_traj_permuted[0,:,:], args, all_three=True)
                #else:
                    #view_traj(ax, pred_traj_gt_permuted[0,:,:], pred_traj_gt_permuted[0,:,:], obs_traj_permuted[0,:,:], args)########changed by cuimingbo
...
def view_traj(ax, fake_pred, real_pred, obs, args, all_three=False):
    ...
    if all_three:
        if(args.dataset_name == 'three_traj'):
            num_traj = 3
        if(args.dataset_name == 'five_traj'):
            num_traj = 5
        x_obs, y_obs, x_pred, y_pred,num = generateTraj(fit_type='square', coeff=0.5, num=num_traj) 
        ...
        elif args.dataset_name == 'five_traj':
            ax.plot(x_pred[1,:], y_pred[1,:], 'r', label='Real Pred 2')
            ax.plot(x_pred[2,:], y_pred[2,:], 'r', label='Real Pred 3')
...
def generator_step(
    ...
    #changed by cuimingbo
    loss += (1- args.l2_loss_weight)*discriminator_loss
    #print(1- args.l2_loss_weight)
